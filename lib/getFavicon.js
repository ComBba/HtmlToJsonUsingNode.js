    ...
    try {
        ...
        try {
            ...
            if (defaultFaviconResponse.status === 200) {
                //console.dir(defaultFaviconResponse);
                //console.log(defaultFaviconResponse.content-type)
                const contentType = defaultFaviconResponse.headers['content-type'];
                if (contentType && contentType.includes('text/html')) {
                    throw new Error('Favicon is a HTML document');
                }
                const buffer = Buffer.from(defaultFaviconResponse.data);

                // Define your size limit here (in bytes)
                const sizeLimit = 1024 * 1024; // for 1MB

                // Check if data size is 0
                if (buffer.length === 0) {
                    console.error(`[Error] Favicon data size is 0.`);
                } else if (buffer.length <= sizeLimit) {
                    return buffer.toString('base64');
                } else {
                    console.error(`[Error] Favicon size exceeds the limit of ${sizeLimit} bytes.`);
                }
            }
        } catch (error) {
        ...
        try {
            ...
            for (const element of linkElements) {
                ...
                try {
                    ...
                    //console.log(`[INFO][Favicon] Status: ${faviconResponse.status}`);
                    switch (faviconResponse.status) {
                        case 200: // OK
                        ...
                        case 304: // Not Modified
                            try {
                                ...
                                console.error(`[Error] Found -> Failed to fetch favicon:`, error);
                            }
                        default:
                            console.info('[INFO][Favicon] Found URL for favicon status: ', faviconResponse.status, ' -> url:', faviconUrl);
                    }
                } catch (error) {
